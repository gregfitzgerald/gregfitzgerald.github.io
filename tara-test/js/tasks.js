...
export function renderSmart() {
  const allPriority = [...SMART_TASKS, ...state.userTasks.filter(t => t.priority)];
  const remaining = allPriority.filter(t => !state.smartDone[t.id]).length;
...
export function renderAsmr() {
  const allTasks = [...ASMR_TASKS, ...state.userTasks.filter(t => !t.priority)];
...
export function renderTasksTab() {
  ...
  const allPriority = [...SMART_TASKS, ...state.userTasks.filter(t => t.priority)];
  const allBacklog = [...ASMR_TASKS, ...state.userTasks.filter(t => !t.priority)];
  ...
  container.innerHTML = `
    ...
    <div class="sidebar-section">
      <div class="sidebar-title">PRIORITY TASKS <span class="badge" id="smart-badge-tab">${allPriority.filter(t => !state.smartDone[t.id]).length}</span></div>
      <div id="smart-list-tab"></div>
    </div>
    <div class="sidebar-section">
      <div class="sidebar-title">BACKLOG TASKS <span class="badge-orange" id="asmr-badge-tab">${allBacklog.filter(t => !state.asmrDone[t.id]).length}</span></div>
